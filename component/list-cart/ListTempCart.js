...
const ListTempCart = () => {
    ...
    if (cart !== undefined) {
      ...
      axios
        ...
        .then((res) => {
          ...
          dispatch({ type: "ADD_TO_CART", total: temp.length });
        })
        .catch((err) => {
  ...
  const updateProduct = async (index, newQuantity) => {
    let tempCart = JSON.parse(cart);

    // data
    tempCart[index].quantity = Number(newQuantity);
    Cookies.set("_cart", JSON.stringify(tempCart));
  ...
  const deleteProduct = async (index) => {
    let tempCart = JSON.parse(cart);
    // DOM
    let newData = data.filter((value, position) => position !== index);
    setData(newData);
    setTotal(convert(String(countTotal(newData))));
          ...
          <div className={classes.list}>
            ...
            <div className={classes.option}>
              ...
              <div className={classes.right}>
                <Link to="/checkout-guest">Proceed to checkout</Link>
                {arrowRight}
              </div>
            </div>
          </div>
